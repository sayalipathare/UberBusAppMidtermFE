...
function useApiFormWithValidation(dataSource, validation, transform) {
    ...
    var _b = React.useState(useApiForm_1.FormStatus.Loading), status = _b[0], setStatus = _b[1];
    React.useEffect(function () {
        setStatus(useApiForm_1.FormStatus.Loading);
        ...
        data.then(function (response) {
            if (response.ok) {
                response.json().then(function (y) {
                    ...
                    setStatus(isValid ? useApiForm_1.FormStatus.Valid : useApiForm_1.FormStatus.ErrorValidation);
                });
            }
            else {
                setStatus(useApiForm_1.FormStatus.ErrorLoading);
            }
        });
    }, [dataSource]);
    useEffectWithDebounce_1.useEffectWithDebounce(function () {
        if (status !== useApiForm_1.FormStatus.Loading && status !== useApiForm_1.FormStatus.ErrorLoading) {
            setStatus(isValid ? useApiForm_1.FormStatus.Valid : useApiForm_1.FormStatus.ErrorValidation);
        }
    }, 300, [isValid, errors]);
